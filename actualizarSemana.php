...
echo "Actualizacion de la semana. ";
...
$dia = date("Ymd", strtotime("-3 day"));

$hora = date("His"); $fecha = date("Ymd");

$semana = date("Ymd", strtotime("-1 week"));
...
$error = 0;
    while (odbc_fetch_row($res)) {
        ...
        $res2 = $con->query("Insert into validar values($numorden, $fecorden, $horaorden, $estorden,
                                                            $estactua, $estanter, $correl, $feccambio,
                                                            $horcambio, $coddesp, '$usuario', '$campo2', $pxq, $depto1)");

        if(!$res2){
            echo "En actualizar validar " . $con->error;
            echo "En mientras " . $error;
            return false;
        }
        $error++;
    }
...
if(actualizarValidar($con, $conexion, $dia, $semana))
    ...
    echo " Hubieron errores " . $con->error;

if(actualizarTiempos($con, $conexion, $dia, $semana))
    echo " Se actualizo tabla tiempo y tabla tiempo.";
else
    echo " Hubieron errores " . $con->error;
...
echo "<br><br>Actualizacion de Resultados de la semana. ";
...
for($dia2 = new DateTime($dia); $dia2->format("Ymd") >= $semana; $dia2->modify("-1 day")) {
    $d = $dia2->format("Ymd");
    echo " Se actualizara el dia " . $d . " <br> ";
...
echo "Se demoro " . ((date("H")*3600+date("i")*60+date("s")) - $ini) . " segundos";
